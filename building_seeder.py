...
def monday_get_buildings():
    ...
    {
      boards(ids: %d) {
        items_page(limit: 500, query_params: {rules: [{column_id: "color", compare_value: ["0"]}]}) {
          items {
            id
            name
            column_values(ids: ["text8","numbers9","numeric_mkt53ys4","numeric_mkt5dq31","status1","location"]) {
              id
              text
            }
          }
